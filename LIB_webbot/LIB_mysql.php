...
#                                                                       
#    update()                                                           
#               Update all rows of a specific column                  
#
#    search()                                                          
#               Executes a SELECT FROM SQL command and return a result set  
#                                                                       
#    exe_sql()                                                          
#               Executes a SQL command such as create table          
#                                                                       
########################################################################
...
----------------------------------------------------------------------*/
define("MYSQL_ADDRESS", "localhost"); //Define the IP address of MySQL Server
define("MYSQL_USERNAME", "root");         // Define your MySQL user name
define("MYSQL_PASSWORD", "mysql");         // Define your MySQL password
define("DATABASE", "stock");               // Define your default database
...
function update($database, $table, $data_array, $key_column, $id) {
  ...
  # Create column and data values for SQL command
	...
	for ($xx=0; $xx<count($data_array); $xx++) {
		  ...
		  if ($xx!=count($data_array)-1) {$setting_list .= ",";}
		  }
...
/***********************************************************************
update_all($database, $table, $data_array)                
-------------------------------------------------------------           
DESCRIPTION:                                                            
        Update all rows of a specific column         
INPUT:                                                                  
        ...
        $table        Table where row insertion occurs                  
        $data_array   A keyed array with defines the data to insert     
        Ex :$data_array["status"]="X";  
            update_all(DATABASE, "stocks_list", $data_array);          
RETURNS                                                                 
        SUCCESS or FAILURE                                              
***********************************************************************/
function update_all($database, $table, $data_array) {
  # Connect to MySQL server and select database
	$mysql_connect = connect_to_database();
	$bool= mysql_select_db ($database, $mysql_connect);
...
function search($database, $sql) {
  ...
  if (mysql_error ($mysql_connect)) {
     ...
     if (sizeof($result_set)==1) {$result_set=$result_set[0];}        
     return $result_set;
     }
	}
/***********************************************************************
...
-------------------------------------------------------------           
DESCRIPTION:                                                            
        Executes a SQL command such as create/drop table                   
INPUT:                                                                  
        $database     Name of database to operate on                    
        $sql          sql command applied to $database                  
RETURNS                                                                 
...
/***********************************************************************
table_exist($database, $table_name)                                                
-------------------------------------------------------------           
DESCRIPTION:                                                            
        Check if a table exists & returns the result                   
INPUT:                                                                  
        $database     Name of database to operate on                    
        $sql          sql command applied to $database                  
RETURNS                                                                 
        True / False                
***********************************************************************/
function table_exist($database, $table_name) {
  # Connect to MySQL server and select database
	$mysql_connect = connect_to_database();
	mysql_select_db($database, $mysql_connect);
