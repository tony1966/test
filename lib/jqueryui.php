...
function get_datatable($id, $url, $column, $scrollY="400px",  
                       ...
                       $lengthMenu=[[15,50,100,-1],[15,50,60,"全部"]],
                       $locale="zh-TW")
功能 : 
...
範例 :
  ...
  echo get_datatable("table1", $url, $column, "", true, "two");
  ...
  echo json_encode($arr); //將陣列轉成 JSON 資料格式傳回  
-----------------------------------------------------------------------------*/
function get_datatable($id, $url, $column, $scrollY="400px", $paginate=false,   
                       $page_buttons="full", $jqueryui=false, 
                       $locale="zh-TW") {
  $opt=Array();
  if ($locale=="zh-TW") { //繁體中文化
      $opt[]='"oLanguage":{"sProcessing":"處理中...",'.
             ...
             '"sNext":"下頁",'.
             '"sLast":"尾頁"}}';
      $opt[]='"aLengthMenu":[[10,25,50,100,-1],[10,25,50,100,"全部"]]';
      }
  else {$opt[]='"aLengthMenu":[[10,25,50,100,-1],[10,25,50,100,"All"]]';}
  if ($jqueryui) {$opt[]='"bJQueryUI":true';} //此參數預設值 false
  if ($scrollY!="") { //有設定垂直卷軸高度
      $opt[]='"sScrollY":"'.$scrollY.'"';
      $opt[]='"bScrollCollapse":true'; //資料長度不足時, 高度設定瓦解
      } 
  if ($paginate) { //此參數預設值 true
      if ($page_buttons==""||$page_buttons=="full") { 
          $opt[]='"sPaginationType":"full_numbers"'; 
          } //此參數預設 "two_button"
      }
  else {$opt[]='"bPaginate":false';}
  $opt[]='"bProcessing":true'; //處理中
  $columns=array();
